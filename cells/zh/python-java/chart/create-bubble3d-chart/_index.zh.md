...
url: /zh/python-java/create-bubble3d-chart/
description: Python 使用 Python 库将 Bubble3D 图表创建到 Excel 的示例代码。使用此代码在基于 Python 的应用程序中为 MS Excel 创建 Bubble3D 图表。
---
...
在运行的应用程序中动态创建 Bubble3D 图表很容易。为了在不需要 MS Office 的情况下从头开始创建各种图表到电子表格，我们将使用 [Aspose.Cells 代表 Python](https://pypi.org/project/aspose.cells)  API，它为在 Python 平台上创建、操作和转换电子表格提供不同的功能。 Aspose.Cells 提供了许多灵活的图表对象。
...
1. 在您的代码文件中导入 asposecells。1. 创建工作簿类实例。1. 将一些数据添加到工作表。1. 将 Bubble3D 图表添加到工作表1. 通过传递其索引来访问 Charts 集合中的新图表对象。1. 使用 Chart.setChartDataRange 方法设置图表的数据源。1. 另存为 Excel 文件。
