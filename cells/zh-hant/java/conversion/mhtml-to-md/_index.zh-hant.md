...
description: Aspose Excel。此綜合解決方案為 Java 開發人員提供了一種完全整合的方法，可將 MHTML 轉換為 MD 格式，從而能夠使用 Aspose.Cells 庫將 MHTML 資料無縫保存為 MD 格式，所有這些都透過高效且可自訂程式碼。
...
{{< blocks/products/pf/i18n/upper-banner h1="將 MHTML 轉換為 MD，得到 Java" h2="高速 Java 庫，用於將 MHTML 轉換為 MD 格式。這是一個專業的軟體解決方案，可使用 Java 匯入和匯出 MHTML、MD 以及許多其他格式。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/cells/aspose_cells-for-java.svg" sourceAdditionalConversionTag="" additionalConversionTag="MD" pfName="" subTitlepfName="" downloadUrl="" fileiconsmall1="DOCX" fileiconsmall2="JPG" fileiconsmall3="PDF" fileiconsmall4="XML" fileiconsmall5="CSV" >}}
...
如何將 MHTML 轉換為 MD？使用 Aspose.Cells for Java 函式庫，只需幾行程式碼即可輕鬆地透過程式設計方式將 MHTML 轉換為 MD。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
{{% blocks/products/pf/agp/content h2="將 MHTML 儲存到 MD，位址為 Java" %}}
...
需要以程式設計方式將 MHTML 檔案轉換為 MD 格式嗎？ Java 開發人員只需幾行程式碼即可輕鬆將 MHTML 轉換為 MD 格式。
...
{{% blocks/products/pf/agp/content h2="使用 Java 庫將 MHTML 轉換為 MD" %}}
{{% blocks/products/pf/agp/text %}}
