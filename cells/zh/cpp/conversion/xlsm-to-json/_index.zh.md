...
description: Aspose Excel。此综合解决方案为 C++ 开发人员提供了一种完全集成的方法，可将 XLSM 转换为 JSON 格式，从而能够使用 Aspose.Cells 库将 XLSM 数据无缝保存为 JSON 格式，所有这些都通过高效且可定制的 C++ 代码实现。
...
如何将 XLSM 转换为 JSON？使用 Aspose.Cells for C++ 库，您只需几行代码即可以编程方式轻松将 XLSM 转换为 JSON。[Aspose.Cells for C++](https://products.aspose.com/cells/cpp)能够构建跨平台应用程序，能够生成、修改、转换、呈现和打印所有 Excel 文件。C++ Excel API 不仅可以在电子表格格式之间进行转换，还可以将 Excel 文件呈现为图像，PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等等，因此它是以行业标准格式交换文档的完美选择。您可以直接下载其最新版本，只需打开[NuGet](https://www.nuget.org/packages/Aspose.Cells.Cpp/)包管理器，搜索 Aspose.Cells.Cpp 并安装。您也可以从包管理器控制台使用以下命令。
...
{{% blocks/products/pf/agp/content h2="将 XLSM 保存到 JSON，即 C++" %}}

以下示例演示了如何在 C++ 中将 XLSM 转换为 JSON。
{{% blocks/products/pf/agp/text %}}

按照简单的步骤将 XLSM 转换为 JSON。上传您的 XLSM 文件，然后将其另存为 JSON 文件。对于 XLSM 读取和 JSON 写入，您都可以使用完全合格的文件名。输出 JSON 内容和格式将与原始 XLSM 文档相同。

{{% /blocks/products/pf/agp/text %}}

{{% /blocks/products/pf/agp/content %}}

{{% cells-convert title="将 XLSM 转换为 JSON 的示例代码" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx;numbers;azw3" InputFormat="xlsm" outformat="json" IsLandingPage="true" language="cpp" %}}

{{% blocks/products/pf/agp/content h2="如何将 XLSM 通过 C++ 转换为 JSON" %}}

{{% blocks/products/pf/agp/text %}}

需要通过编程方式将 XLSM 文件转换为 JSON 文件吗？C++ 开发人员只需几行代码即可轻松将 XLSM 转换为 JSON。
...
1. 向您的 C++ 项目添加库引用（导入库）。
1. 使用工作簿加载 XLSM 文件。
1. 调用 Save() 方法将 XLSM 转换为 JSON。
1. 得到XLSM至JSON的转换结果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="使用 C++ 库将 XLSM 转换为 JSON" %}}
