---
title: Java XLSM 到 SQL - XLSM 到 SQL 轉換器
description: Aspose Excel。使用 Aspose.Cells. Java XLSM 到 SQL 快速輕鬆地將 XLSM 轉換為 SQL。 Java 將 XLSM 儲存到 SQL。使用 Java 將 XLSM 儲存為 SQL。
...
{{% blocks/products/pf/agp/content h2="使用 Java 將 XLSM 轉換為 SQL" %}}

如何將 XLSM 轉換為 SQL？使用 Aspose.Cells for Java 函式庫，您可以透過幾行程式碼以程式設計方式輕鬆將 XLSM 轉換為 SQL。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
{{% blocks/products/pf/agp/content h2="將XLSM儲存到SQL中Java" %}}

以下範例示範如何將 XLSM 轉換為 SQL 中的 Java。
...
{{% cells-convert title="將 XLSM 轉換為 SQL 的範例程式碼" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx" InputFormat="xlsm" outformat="sql" IsLandingPage="true" language="java" %}}

{{% blocks/products/pf/agp/content h2="如何將 XLSM 轉換為 SQL via Java" %}}

{{% blocks/products/pf/agp/text %}}

需要以程式設計方式將 XLSM 檔案轉換為 SQL？ Java開發人員只需幾行程式碼即可輕鬆將XLSM轉換為SQL。
...
1. 透過呼叫 Workbook.save 方法將 XLSM 轉換為 SQL。
1. 取得XLSM到SQL的轉換結果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="Java函式庫將XLSM轉換為SQL" %}}
{{% blocks/products/pf/agp/text %}}
