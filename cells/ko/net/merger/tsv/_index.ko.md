...
{{% blocks/products/pf/agp/content h2="C#을 사용하여 TSV 파일을 병합하는 방법" %}}
 ...
 TSV 파일을 병합하기 위해 다음을 사용합니다.[Aspose.Cells for .NET](https://products.aspose.com/cells/net) API은 기능이 풍부하고 강력하며 사용하기 쉬운 문서 조작 및 C# 플랫폼용 병합 API입니다. 열려 있는[NuGet](https://www.nuget.org/packages/aspose.cells) 패키지 관리자, 검색**Aspose.Cells** 그리고 설치하세요. 패키지 관리자 콘솔에서 다음 명령을 사용할 수도 있습니다.
...
+ 전체 경로가 포함된 모든 TSV 파일을 로드합니다.
+ 하나의 문서를 기본 파일로 만들기
+ 파일을 하나씩 연결하고 병합하는 해당 메소드를 호출합니다.
+ Save() 메서드를 호출하고 파일 이름(전체 경로)과 형식(TSV)을 매개 변수로 전달합니다.
+ 이제 Microsoft Office, Adobe PDF 또는 기타 호환 프로그램에서 TSV 파일을 열고 사용할 수 있습니다.
