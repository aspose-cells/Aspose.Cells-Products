---
title: Python JSON 轉 FODS - 在線 JSON 轉 FODS 轉換器
description: Aspose Excel。 Python Aspose.Cells。Python Aspose Cells。免費在線 Python 將 JSON 轉換為 FODS 保存格式。 Python JSON 到 FODS 格式。將 JSON 保存為 FODS Python。
...
{{< blocks/products/pf/i18n/upper-banner h1="將 JSON 轉換為 Python 中的 FODS" h2="高速 Python 庫，用於將 JSON 轉換為 FODS。使用我們的 Excel 轉換 API 在 Python 中開發高級、平台獨立的軟件。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/cells/aspose_cells-for-python-net.svg" sourceAdditionalConversionTag="" additionalConversionTag="FODS" pfName="" subTitlepfName="" downloadUrl="" fileiconsmall1="HTML" fileiconsmall2="JPG" fileiconsmall3="PDF" fileiconsmall4="XML" fileiconsmall5="XLSX" >}}
...
如何將 JSON 轉換為 FODS？使用 Aspose.Cells for Python via NET 庫，您可以通過幾行代碼以編程方式輕鬆將 JSON 轉換為 FODS。[Aspose.Cells for Python 通過 NET](https://pypi.org/project/aspose-cells-python/)能夠構建跨平台應用程序，能夠生成、修改、轉換、渲染和打印所有 Excel 文件。 Python Excel API不僅可以在電子表格格式之間進行轉換，還可以將Excel文件渲染為圖像，PDF、HTML、ODS、CSV、SVG、JSON、WORD、PPT等，從而使其成為行業交換文檔的完美選擇-標準格式。

{{% /blocks/products/pf/agp/content %}}


{{% blocks/products/pf/agp/content h2="免費在線保存 JSON 至 FODS 於 Python" %}}

以下示例演示如何將 Python via NET 中的 JSON 轉換為 FODS。
...
需要以編程方式將 JSON 文件轉換為 FODS 嗎？ Python 開發人員只需幾行代碼即可輕鬆加載和轉換 JSON 到 FODS。
...
1. 使用工作簿實例加載 JSON 文件。
1. 通過調用Workbook.save方法將JSON轉換為FODS。
1. 獲取JSON到FODS的轉換結果。
