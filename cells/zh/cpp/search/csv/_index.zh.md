...
weight: 5540
description: C++ 示例代码，用于在 CSV 文件中使用模式搜索单词，适用于 C++ 运行时环境，支持 Windows 32 位、Windows 64 位和 Linux 64 位。
...
{{% blocks/products/pf/agp/code-block title="搜索文件 CSV - C++" offSpacer="" %}}
...
{{< blocks/products/pf/agp/about-file-section >}}
    ...
    {{< blocks/products/pf/agp/demobox sectionTitle="在线 CSV 搜索实时演示" sectionDescription="立即访问我们的网站，搜索编号为 CSV 的文档中的文本、单词和短语。[现场演示网站](https://products.aspose.app/cells/search)现场演示有以下好处" >}}
...
扩展名为 CSV（逗号分隔值）的文件是纯文本文件，其中包含以逗号分隔的数据记录。CSV 文件中的每一行都是该文件所包含记录集中的一条新记录。当需要将数据从一个存储系统传输到另一个存储系统时，就会生成此类文件。由于所有应用程序都能识别以逗号分隔的记录，因此将此类数据文件导入数据库非常方便。几乎所有电子表格应用程序，例如 Microsoft Excel 或 OpenOffice Calc，都可以轻松导入 CSV 文件。从此类文件导入的数据会排列在电子表格的单元格中，以便用户查看。
