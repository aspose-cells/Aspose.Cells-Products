...
在運行的應用程序中動態創建散點圖很容易。為了在不需要 MS Office 的情況下從頭開始創建各種圖表到電子表格，我們將使用[Aspose.Cells for .NET](https://products.aspose.com/cells/net)API，使用 .NET 平台為電子表格創建、操作和轉換提供不同的功能。 Aspose.Cells 提供了許多靈活的圖表對象。
...
開發人員只需幾行代碼就可以輕鬆地在運行不同的報告應用程序中創建散點圖以進行數據處理。
