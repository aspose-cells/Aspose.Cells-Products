---
title: C# SQLite 到 EXCEL - SQLite 到 EXCEL 转换器
description: Aspose Excel。使用 Aspose.Cells 快速轻松地将 SQLite 转换为 EXCEL。C# SQLite 到 EXCEL。 C# 将 SQLite 保存到 EXCEL。使用 C# 将 SQLite 保存为 EXCEL。
...
{{% blocks/products/pf/agp/content h2="如何通过 C# 将 SQLite 转换为 EXCEL" %}}

{{% blocks/products/pf/agp/text %}}

需要以编程方式将 SQLite 数据转换为 EXCEL？ .NET 开发人员只需几行代码即可轻松加载 SQLite 并将其转换为 EXCEL。

{{% /blocks/products/pf/agp/text %}}

1. 安装“Aspose.Cells for .NET”。
1. 将库引用（导入库）添加到您的 C# 项目中。
1. 从SQLite数据库中查询数据，获取DataTable对象。
1. 创建一个新的工作簿并从 DataTable 对象导入数据。
1. 通过调用Workbook.Save方法将数据保存为xlsx格式。

{{% blocks/products/pf/agp/code-block title="将 SQLite 转换为 EXCEL 的示例代码 - C#" offSpacer="" %}}
...
{{% blocks/products/pf/agp/content h2="C# 将 SQLite 转换为 EXCEL 的库" %}}
