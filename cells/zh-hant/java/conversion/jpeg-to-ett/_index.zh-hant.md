---
title: Java JPEG 轉 ETT - JPEG 轉 ETT 轉換器
description: Aspose Excel。使用 Aspose.Cells 快速輕鬆地將 JPEG 轉換為 ETT。Java JPEG 轉換為 ETT。 Java 將 JPEG 儲存至 ETT。使用 Java 將 JPEG 儲存為 ETT。
...
{{% blocks/products/pf/agp/content h2="使用 Java 將 JPEG 轉換為 ETT" %}}

如何將 JPEG 轉換為 ETT？借助 Aspose.Cells for Java 庫，您可以透過幾行程式碼以程式設計方式輕鬆將 JPEG 轉換為 ETT。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
{{% blocks/products/pf/agp/content h2="將 JPEG 儲存到 Java 中的 ETT" %}}

以下範例示範如何將 JPEG 轉換為 Java 中的 ETT。
...
{{% cells-convert title="將 JPEG 轉換為 ETT 的範例程式碼" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx" InputFormat="jpg" outformat="ett" IsLandingPage="true" language="java" %}}

{{% blocks/products/pf/agp/content h2="如何將 JPEG 轉換為 ETT via Java" %}}

{{% blocks/products/pf/agp/text %}}

需要以程式方式將 JPEG 檔案轉換為 ETT？ Java 開發人員只需幾行程式碼即可輕鬆將 JPEG 轉換為 ETT。
...
{{% blocks/products/pf/agp/content h2="Java 庫將 JPEG 轉換為 ETT" %}}
{{% blocks/products/pf/agp/text %}}
