---
title: PHP SXC 轉 TXT - SXC 轉 TXT 轉換器
description: Aspose Excel。此綜合解決方案為 PHP 開發人員提供了一種完全整合的方法，可將 SXC 轉換為 TXT 格式，從而能夠使用 Aspose.Cells 庫將 SXC 資料無縫保存為 TXT 格式，所有這些程式碼可實現且可自訂的 PHP 都通過高效設定。
...
如何將 SXC 轉換為 TXT？使用 Aspose.Cells for PHP via Java 函式庫，只需幾行程式碼即可輕鬆地透過程式設計方式將 SXC 轉換為 TXT。[Aspose.Cells for PHP via Java](https://products.aspose.com/cells/php-java/)它能夠建立跨平台應用程序，並具備生成、修改、轉換、渲染和列印所有 Excel 文件的能力。 PHP Excel API 不僅可以轉換電子表格格式，還可以將 Excel 文件渲染為圖像（PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT）等等，因此是交換行業標準格式文件的理想選擇。
 
{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="將 SXC 儲存為 TXT（PHP 格式）" %}}
...
需要透過程式將 SXC 檔案轉換為 TXT 檔案嗎？ PHP 開發人員只需幾行程式碼即可輕鬆載入 SXC 檔案並將其轉換為 TXT 檔案。
...
{{% blocks/products/pf/agp/content h2="用於將 SXC 轉換為 TXT 的 PHP 函式庫" %}}
