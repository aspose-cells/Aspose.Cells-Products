---
title: C++ EXCEL 轉 OTS - EXCEL 轉 OTS 轉換器
description: Aspose Excel。此綜合解決方案為 C++ 開發人員提供了一種完全整合的方法，可將 EXCEL 轉換為 OTS 格式，從而能夠使用 Aspose.Cells 庫將 EXCEL 資料無縫儲存為 OTS 格式，所有這些都透過高效且可自訂的 C++ 程式碼實現。
...
如何將 EXCEL 轉換為 OTS？借助 Aspose.Cells for C++ 庫，您可以透過幾行程式碼以程式設計方式輕鬆將 EXCEL 轉換為 OTS。[Aspose.Cells for C++](https://products.aspose.com/cells/cpp)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 C++ Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接下載最新版本，打開即可[NuGet](https://www.nuget.org/packages/Aspose.Cells.Cpp/)套件管理器，搜尋Aspose.Cells.cpp並安裝。您也可以從套件管理器控制台使用以下命令。
...
{{% blocks/products/pf/agp/content h2="將 EXCEL 檔案儲存到 OTS 系統，檔案編號為 C++" %}}

以下範例示範如何在 C++ 中將 EXCEL 轉換為 OTS。
...
{{% blocks/products/pf/agp/content h2="如何透過 C++ 將 EXCEL 轉換為 OTS" %}}

{{% blocks/products/pf/agp/text %}}

需要透過程式設計方式將 EXCEL 檔案轉換為 OTS 格式嗎？ C++ 開發人員只需幾行程式碼即可輕鬆將 EXCEL 檔案轉換為 OTS 格式。
...
1. 使用工作簿載入 EXCEL 檔案。
1. 透過呼叫 Save() 方法將 EXCEL 轉換為 OTS。
1. 取得EXCEL到OTS的轉換結果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="C++ 庫，用於將 EXCEL 轉換為 OTS" %}}
