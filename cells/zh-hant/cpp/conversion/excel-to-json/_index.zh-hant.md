---
title: C++ EXCEL 到 JSON - EXCEL 到 JSON 轉換器
description: Aspose Excel。此綜合解決方案為 C++ 開發人員提供了一種完全整合的方法，可將 EXCEL 轉換為 JSON 格式，從而能夠使用 Aspose.Cells 庫將 EXCEL 資料無縫保存為 JSON 格式，所有這些程式庫將 EXCEL 資料無縫儲存為 JSON 格式，所有這些程式碼都透過高效且可自訂且可自訂153 153 153 153。
...
如何將 EXCEL 轉換為 JSON？使用 Aspose.Cells for C++ 函式庫，您可以透過幾行程式碼以程式設計方式輕鬆將 EXCEL 轉換為 JSON。[Aspose.Cells for C++](https://products.aspose.com/cells/cpp)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 C++ Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接下載最新版本，打開即可[NuGet](https://www.nuget.org/packages/Aspose.Cells.Cpp/)套件管理器，搜尋Aspose.Cells.cpp並安裝。您也可以從套件管理器控制台使用以下命令。
...
{{% blocks/products/pf/agp/content h2="將 Excel 檔案儲存到 JSON（位於 C++ 中）" %}}

以下範例示範如何將 EXCEL 轉換為 JSON 到 C++。
...
{{% cells-convert title="將 EXCEL 轉換為 JSON 的範例程式碼" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx;numbers;azw3" InputFormat="xlsx" outformat="json" IsLandingPage="true" language="cpp" %}}

{{% blocks/products/pf/agp/content h2="如何透過 C++ 將 EXCEL 轉換為 JSON" %}}

{{% blocks/products/pf/agp/text %}}

需要透過程式將 EXCEL 檔案轉換為 JSON 格式嗎？ C++ 開發人員只需幾行程式碼即可輕鬆將 EXCEL 檔案轉換為 JSON 格式。
...
1. 使用工作簿載入 EXCEL 檔案。
1. 透過呼叫 Save() 方法將 EXCEL 轉換為 JSON。
1. 得到EXCEL轉換結果為JSON。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="C++ 庫用於將 EXCEL 轉換為 JSON" %}}
...
{{< blocks/products/pf/agp/other-supported-section-item href="https://products.aspose.com/cells/cpp/conversion/excel-to-tiff/" name="Excel至TIFF" description="標記影像格式" >}}
