...
description: Aspose Excel。此綜合解決方案為 C++ 開發人員提供了一種完全整合的 EXCEL 到 EXCEL 格式轉換方法，能夠使用 Aspose.Cells 庫將 EXCEL 資料無縫儲存為 EXCEL 格式，所有這些都透過高效且可自訂的 C++ 程式碼實現。
...
{{% blocks/products/pf/agp/content h2="將 EXCEL 儲存到 EXCEL（C++）" %}}

以下範例示範如何在 C++ 中將 EXCEL 轉換為 EXCEL。
...
需要透過程式將 EXCEL 檔案轉換為 EXCEL 檔案嗎？ C++ 開發人員只需幾行程式碼即可輕鬆實現 EXCEL 檔案之間的轉換。
...
1. 使用工作簿載入 EXCEL 檔案。
1. 透過呼叫 Save() 方法將 EXCEL 檔案轉換為 EXCEL 檔案。
1. 取得EXCEL到EXCEL的轉換結果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="C++ 庫，用於將 EXCEL 轉換為 EXCEL" %}}
