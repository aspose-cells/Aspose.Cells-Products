---
title: 保護和鎖定ODS文檔via Java
weight: 6360
description: Java 範例程式碼，用於在 JSP/JSF 應用程式和桌面應用程式的 Java 執行時間環境上使用密碼鎖定 ODS 檔案。
