---
title: 建立XLTM - 在Java中建立XLTM文件
description: Aspose Excel。 Java 使用 Aspose.Cells 快速輕鬆地建立 XLTM 檔案。使用 Java 產生 XLTM 檔案。在 Java 中建立 XLTM。在 Java 中建立 XLTM。
...
{{< blocks/products/pf/i18n/upper-banner h1="在Java中建立XLTM文件" h2="用於建立 XLTM 檔案的高速 Java 庫。這是一個專業的軟體解決方案，用於導入和導出XLSX、PDF以及使用Java的許多其他格式。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/cells/aspose_cells-for-java.svg" sourceAdditionalConversionTag="" additionalConversionTag="XLTM" pfName="" subTitlepfName="" downloadUrl="" fileiconsmall1="HTML" fileiconsmall2="JPG" fileiconsmall3="PDF" fileiconsmall4="XML" fileiconsmall5="XLSX" >}}
...
{{% blocks/products/pf/agp/content h2="使用 Java 建立 XLTM 文件" %}}

如何建立XLTM檔案？使用 Aspose.Cells for Java 庫，您可以透過幾行程式碼以程式設計方式輕鬆建立 XLTM 檔案。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
{{% blocks/products/pf/agp/content h2="如何在Java中建立XLTM" %}}
...
1. 使用 save() 方法將工作簿儲存為 XLTM 格式。

{{% blocks/products/pf/agp/code-block title="範例程式碼展示如何在 Java 中建立 XLTM 檔案。" offSpacer="" %}}
...
{{% blocks/products/pf/agp/content h2="Java庫建立XLTM文件" %}}
{{% blocks/products/pf/agp/text %}}
