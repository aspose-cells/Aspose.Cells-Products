...
weight: 7690
description: C# 使用 .NET 库将数据透视图添加到 XLS 的示例代码。使用此代码将数据透视图插入 VB.NET、Asp.NET 或任何基于 .NET 的应用程序中的 XLS 文件。
...
在运行的应用程序中动态生成带有数据透视图的 MS Excel XLS 文件非常简单。为了从头开始创建带有数据透视图的 XLS 文档而不需要 MS Office，我们将使用[Aspose.Cells for .NET](https://products.aspose.com/cells/net)API，使用 .NET 平台提供电子表格创建、操作和转换的不同功能。开发人员可以轻松增强用于写入数据、生成图表或图形以及在电子表格中创建表格的代码。
...
开发人员可以轻松地在运行不同的报告应用程序中插入 XLS 格式电子表格的数据透视图，以进行数据处理，只需几行代码。
