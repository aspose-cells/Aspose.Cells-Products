---
title: Java JPEG 至 CSV - JPEG 至 CSV 轉換器
description: Aspose Excel。 Java Aspose.Cells。Java中的JPEG到CSV。Java將JPEG轉換為CSV儲存格式。 Java JPEG 到 CSV 格式。將 JPEG 儲存為 CSV Java。
...
如何將 JPEG 轉換為 CSV？使用 Aspose.Cells for Java 函式庫，您可以透過幾行程式碼以程式設計方式輕鬆將 JPEG 轉換為 CSV。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
以下範例示範如何將 Java 中的 JPEG 轉換為 CSV。
...
{{% cells-convert title="將 JPEG 轉換為 CSV 的範例程式碼" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx" InputFormat="jpg" outformat="csv" IsLandingPage="true" language="java" %}}

{{% blocks/products/pf/agp/content h2="如何轉換 JPEG 為 CSV via Java" %}}

{{% blocks/products/pf/agp/text %}}

需要以程式方式將 JPEG 檔案轉換為 CSV 嗎？ Java開發人員只需幾行程式碼即可輕鬆將JPEG轉換為CSV。
