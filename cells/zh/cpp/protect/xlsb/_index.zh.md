...
1. 使用 IWorkbook 类加载 XLSB 文件1. 将 Protect(..) 方法与 ProtectionType 和 Password 一起使用1. 通过 Save() 方法保存受保护的 XLSB 文件
