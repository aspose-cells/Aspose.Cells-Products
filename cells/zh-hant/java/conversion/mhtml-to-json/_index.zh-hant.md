---
title: Java MHTML 至 JSON - MHTML 至 JSON 轉換器
description: Aspose Excel。使用 Aspose.Cells 快速輕鬆地將 MHTML 轉換為 JSON。Java MHTML 為 JSON. Java 將 0761838381830 1076181 將 0761830761830 10760181 將 07618307618381 儲存。為 07611134 81 使用 Java。
...
{{< blocks/products/pf/i18n/upper-banner h1="將 MHTML 轉換為 Java 中的 JSON" h2="高速Java庫，用於將MHTML轉換為JSON。這是一個專業的軟體解決方案，用於導入和導出MHTML、JSON和使用Java的許多其他格式。" logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/cells/aspose_cells-for-java.svg" sourceAdditionalConversionTag="" additionalConversionTag="JSON" pfName="" subTitlepfName="" downloadUrl="" fileiconsmall1="DOCX" fileiconsmall2="JPG" fileiconsmall3="PDF" fileiconsmall4="XML" fileiconsmall5="CSV" >}}
...
如何將 MHTML 轉換為 JSON？使用 Aspose.Cells for Java 函式庫，您可以透過幾行程式碼以程式設計方式輕鬆將 MHTML 轉換為 JSON。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
以下範例示範如何將 Java 中的 MHTML 轉換為 JSON。
{{% blocks/products/pf/agp/text %}}

按照簡單的步驟將 MHTML 轉換為 JSON。上傳您的 MHTML 文件，然後只需將其另存為 JSON 文件。對於 MHTML 讀取和 JSON 寫入，您可以使用完全限定的檔案名稱。輸出 JSON 內容和格式將與原始 MHTML 文件相同。

{{% /blocks/products/pf/agp/text %}}

{{% /blocks/products/pf/agp/content %}}

{{% cells-convert title="將 MHTML 轉換為 JSON 的範例程式碼" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx" InputFormat="mhtml" outformat="json" IsLandingPage="true" language="java" %}}

{{% blocks/products/pf/agp/content h2="如何轉換 MHTML 為 JSON via Java" %}}

{{% blocks/products/pf/agp/text %}}

需要以程式方式將 MHTML 檔案轉換為 JSON 嗎？ Java開發人員只需幾行程式碼即可輕鬆將MHTML轉換為JSON。
...
1. 透過呼叫Workbook.save方法將MHTML轉換為JSON。
1. 取得MHTML到JSON的轉換結果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="Java庫將MHTML轉換為JSON" %}}
{{% blocks/products/pf/agp/text %}}
