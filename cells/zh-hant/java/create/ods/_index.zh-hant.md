...
description: Aspose Excel。使用 Aspose.Cells for Java 函式庫，輕鬆產生和操作 ODS 檔案。這款強大的 API 庫提供了一種簡化高效的解決方案，可直接在您的 Java 應用程式中建立、編輯和管理 ODS 文件，從而高精度地滿足開發人員的特定需求。
...
如何建立 ODS 檔案？使用 Aspose.Cells for Java 函式庫，只需幾行程式碼即可輕鬆以程式設計方式建立 ODS 檔案。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
