---
title: C# SQLite から EXCEL - SQLite から EXCEL コンバータ
description: Aspose エクセル。 Aspose.Cells を使用して SQLite を EXCEL にすばやく簡単に変換します。 C# SQLite から EXCEL。 C# SQLite を EXCEL に保存します。 C# を使用して SQLite を EXCEL として保存します。
...
{{% blocks/products/pf/agp/content h2="C# 経由で SQLite を EXCEL に変換する方法" %}}

{{% blocks/products/pf/agp/text %}}

SQLite データをプログラムで EXCEL に変換する必要がありますか? .NET 開発者は、わずか数行のコードで SQLite を簡単にロードして EXCEL に変換できます。

{{% /blocks/products/pf/agp/text %}}

1.  「Aspose.Cells for .NET」をインストールします。
1. ライブラリ参照を C# プロジェクトに追加します (ライブラリをインポートします)。
1.  SQLite データベースからデータをクエリして、DataTable オブジェクトを取得します。
1. 新しいワークブックを作成し、DataTable オブジェクトからデータをインポートします。
1. Workbook.Save メソッドを呼び出して、データを xlsx 形式で保存します。

{{% blocks/products/pf/agp/code-block title="SQLite を EXCEL に変換するサンプル コード - C#" offSpacer="" %}}
...
{{% blocks/products/pf/agp/content h2="C# SQLite を EXCEL に変換するライブラリ" %}}
