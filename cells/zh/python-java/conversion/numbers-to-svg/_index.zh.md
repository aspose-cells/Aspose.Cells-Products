...
按照简单的步骤将 NUMBERS 转换为 SVG。上传您的 NUMBERS 文件，然后只需将其另存为 SVG 文件。对于 NUMBERS 读取和 SVG 写入，您可以使用完全限定的文件名。输出 SVG 内容和格式将与原始 NUMBERS 文档相同。
...
需要以编程方式将 NUMBERS 文件转换为 SVG？ Python 开发人员只需几行代码即可轻松加载 NUMBERS 并将其转换为 SVG。
