...
為了合併 XLSB 文件，我們將使用[Aspose.Cells for Java](https://products.aspose.com/cells/java)API這是一個功能豐富、功能強大且易於使用的合併API for Java平台。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
使用 Workbook 類別的實例載入第一個 XLSB 檔案。
+ 使用 Workbook 類別的實例載入第二個 XLSB 文件。
+ 使用combine() 方法合併文件。
+ 將合併後的XLSB檔案儲存在指定路徑
