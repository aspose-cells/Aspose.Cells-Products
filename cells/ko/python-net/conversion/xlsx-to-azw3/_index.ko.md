---
title: Python XLSX을 AZW3로 변환 - XLSX을 AZW3로 변환
description: Aspose Excel. 이 포괄적인 솔루션은 Python 개발자에게 XLSX을 AZW3 형식으로 변환하는 완벽하게 통합된 접근 방식을 제공하여 Aspose.Cells 라이브러리를 사용하여 XLSX 데이터를 AZW3 형식으로 원활하게 저장할 수 있도록 하며, 이 모든 작업은 효율적이고 사용자 정의 가능한 Python 코드를 통해 이루어집니다.
...
{{% blocks/products/pf/agp/content h2="XLSX을 AZW3 형식으로 변환하는 Excel 라이브러리 (Python)" %}}

 XLSX을 AZW3로 변환하려면 어떻게 해야 하나요? Aspose.Cells, for Python과 같은 라이브러리를 사용하면 몇 줄의 코드로 XLSX을 AZW3로 쉽게 변환할 수 있습니다.[Aspose.Cells for Python 경유 NET](https://pypi.org/project/aspose-cells-python/)모든 Excel 파일을 생성, 수정, 변환, 렌더링 및 인쇄할 수 있는 기능을 갖춘 크로스 플랫폼 애플리케이션을 구축할 수 있습니다. Python Excel API 스프레드시트 형식 간에 변환할 뿐만 아니라 Excel 파일을 이미지(PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 등)로 렌더링할 수도 있으므로 업계 표준 형식으로 문서를 교환하는 데 완벽한 선택입니다.

{{% /blocks/products/pf/agp/content %}}


{{% blocks/products/pf/agp/content h2="XLSX 파일을 Python Excel 라이브러리의 AZW3 형식으로 저장합니다." %}}

다음 예시는 XLSX을 Python 엑셀 라이브러리의 AZW3 형식으로 변환하는 방법을 보여줍니다.
{{% blocks/products/pf/agp/text %}}

다음의 간단한 단계를 따라 XLSX 파일을 AZW3 파일로 변환하세요. XLSX 파일을 업로드한 다음 AZW3 파일로 저장하기만 하면 됩니다. XLSX 파일 읽기 및 AZW3 파일 쓰기 모두에서 전체 경로 파일 이름을 사용할 수 있습니다. 변환된 AZW3 파일의 내용과 서식은 원본 XLSX 문서와 동일합니다.

{{% /blocks/products/pf/agp/text %}}

{{% /blocks/products/pf/agp/content %}}

{{% cells-convert title="Python Excel 라이브러리를 사용하여 XLSX을 AZW3로 변환하는 샘플 코드" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx;numbers;azw3" InputFormat="xlsx" outformat="azw3" IsLandingPage="true" language="pythonnet" %}}

{{% blocks/products/pf/agp/content h2="XLSX을 AZW3로 변환하는 방법" %}}

{{% blocks/products/pf/agp/text %}}

XLSX 파일을 AZW3 형식으로 프로그램적으로 변환해야 하시나요? Python 개발자는 단 몇 줄의 코드로 XLSX 파일을 쉽게 불러와 AZW3로 변환할 수 있습니다.
...
1.  Workbook.save 메서드를 호출하여 XLSX을 AZW3 형식으로 변환합니다.
1.  XLSX을 AZW3로 변환한 결과를 확인하세요.

{{% /blocks/products/pf/agp/content %}}


{{% blocks/products/pf/agp/content h2="Python Excel 라이브러리를 사용하여 XLSX을 AZW3로 변환합니다." %}}
