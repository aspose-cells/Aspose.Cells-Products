---
title: C#을 통해 캔들스틱 차트 생성
description: C# .NET 라이브러리를 사용하여 Excel에 캔들스틱(시가-고가-저가-종가) 차트를 만드는 샘플 코드입니다. 이 코드를 사용하여 VB.NET, Asp.NET 또는 .NET 기반 응용 프로그램 내에서 MS Excel에 대한 캔들스틱 차트를 생성합니다.
---
...
시가-고가-저가-종가(OHLC) 차트는 범주, 시가, 고가, 저가 및 종가 순서대로 5개의 데이터 열을 사용합니다. 각 범주의 가격 범위는 수직선으로 다시 표시되며 시가와 종가 사이의 범위는 더 넓은 부동 막대로 표시됩니다. 범주에서 가격이 상승하면(종가가 시가보다 높음) 막대가 한 가지 색상으로 채워지고 가격이 하락하면 막대가 다른 색상으로 채워집니다. 이러한 유형의 차트는 종종 캔들스틱 차트라고 합니다.
...
{{% /blocks/products/pf/agp/feature-section-col %}}
{{% blocks/products/pf/agp/code-block title="다음 소스 코드는 C#을 사용하여 MS Excel XLSX 파일에 캔들스틱(시가-고가-저가-종가) 차트를 만드는 방법을 보여줍니다." offSpacer="" %}}
