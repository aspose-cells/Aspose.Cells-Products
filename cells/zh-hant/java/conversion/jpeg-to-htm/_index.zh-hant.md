---
title: Java JPEG 轉 HTM - JPEG 轉 HTM 轉換器
description: Aspose Excel。此綜合解決方案為 Java 開發人員提供了一種完全整合的方法，可將 JPEG 轉換為 HTM 格式，從而能夠使用 Aspose.Cells 庫將 JPEG 資料無縫保存為 HTM 格式，所有這些都透過高效能客製化的 081 53481 實現程式碼。
...
如何將 JPEG 轉換為 HTM？使用 Aspose.Cells for Java 函式庫，只需幾行程式碼即可輕鬆地透過程式設計方式將 JPEG 轉換為 HTM。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
{{% blocks/products/pf/agp/content h2="將 JPEG 儲存到 HTM，即 Java" %}}

以下範例示範如何將 JPEG 轉換為 Java 中的 HTM。
...
需要透過程式將 JPEG 檔案轉換為 HTM 格式嗎？ Java 開發人員只需幾行程式碼即可輕鬆將 JPEG 轉換為 HTM 格式。
...
1. 呼叫 Workbook.save 方法將 JPEG 轉換為 HTM。
1. 取得 JPEG 到 HTM 的轉換結果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="使用 Java 庫將 JPEG 轉換為 HTM" %}}
{{% blocks/products/pf/agp/text %}}
