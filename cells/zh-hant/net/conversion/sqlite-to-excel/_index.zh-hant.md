---
title: C# SQLite 到 EXCEL - SQLite 到 EXCEL 轉換器
description: Aspose Excel。使用 Aspose.Cells 快速輕鬆地將 SQLite 轉換為 EXCEL。C# SQLite 到 EXCEL。 C# 將 SQLite 儲存到 EXCEL。使用 C# 將 SQLite 儲存為 EXCEL。
...
{{% blocks/products/pf/agp/content h2="如何透過 C# 將 SQLite 轉換為 EXCEL" %}}

{{% blocks/products/pf/agp/text %}}

需要以程式設計方式將 SQLite 資料轉換為 EXCEL？ .NET 開發人員只需幾行程式碼即可輕鬆載入 SQLite 並將其轉換為 EXCEL。

{{% /blocks/products/pf/agp/text %}}

1. 安裝“Aspose.Cells for .NET”。
1. 將庫引用（導入庫）新增至您的 C# 專案中。
1. 從SQLite資料庫中查詢數據，取得DataTable物件。
1. 建立一個新的工作簿並從 DataTable 物件匯入資料。
1. 透過呼叫Workbook.Save方法將資料儲存為xlsx格式。

{{% blocks/products/pf/agp/code-block title="將 SQLite 轉換為 EXCEL 的範例程式碼 - C#" offSpacer="" %}}
...
{{% blocks/products/pf/agp/content h2="C# 將 SQLite 轉換為 EXCEL 的函式庫" %}}
