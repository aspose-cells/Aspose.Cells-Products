---
title: 將 SPREADSHEETML 合併到 JSON - 將 SPREADSHEETML 合併到 Java 中的 JSON
url: /zh-hant/java/merger/merge-spreadsheetml-to-json/ 
description: Aspose Excel。 Java Aspose.Cells。將 SPREADSHEETML 合併到 JSON 儲存格式。 Java 將 SPREADSHEETML 合併為 JSON 格式。將 SPREADSHEETML 合併到 Java 中的 JSON。SPREADSHEETML 合併。
...
{{% blocks/products/pf/agp/content h2="使用 Java 將 SPREADSHEETML 合併到 JSON" %}}

如何將 SPREADSHEETML 合併到 JSON？使用 Aspose.Cells for Java 函式庫，您可以透過幾行程式碼以程式設計方式輕鬆將 SPREADSHEETML 合併到 JSON。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能夠建立跨平台應用程序，能夠生成、修改、轉換、渲染和列印所有 Excel 檔案。 Java Excel API 不僅可以在電子表格格式之間進行轉換，還可以將 Excel 文件呈現為圖像、PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT 等，從而使其成為以行業標準格式交換文件的完美選擇。您可以直接從以下位置下載其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)並透過將以下配置新增至 pom.xml 將其安裝在基於 Maven 的專案中。
...
{{% blocks/products/pf/agp/content h2="將 SPREADSHEETML 合併到 Java 中的 JSON" %}}

以下範例示範如何將 SPREADSHEETML 合併到 Aspose.Cells for Java 中的 JSON。
{{% blocks/products/pf/agp/text %}}

按照簡單的步驟將 SPREADSHEETML 合併到 JSON。上傳文件，呼叫 Workbook.Combine 方法合併文件，然後將其儲存到 JSON 文件。如果您在 Java 中開發程式碼，這會比聽起來更簡單。請參閱 Java 將 SPREADSHEETML 合併到 JSON 的範例。

{{% /blocks/products/pf/agp/text %}}

{{% /blocks/products/pf/agp/content %}}

{{% cells-merger title="將 SPREADSHEETML 合併到 Java 中的 JSON 的範例程式碼" InputFormat="xml" InputFormat2="json" outformat="json" formats="pdf;xls;xlsx;docx;pptx;xlsm;xlsb;xlt;ods;ots;csv;tsv;html;htm;mht;numbers;jpg;bmp;png;webp;svg;tiff;xps;mhtml;md;json;xml;zip;sql;txt;et;ett;prn;dif;emf;fods;gif;sxc;xlam;xltm;xltx" IsLandingPage="true" language="java" %}}

{{% blocks/products/pf/agp/content h2="如何將 SPREADSHEETML 合併到 JSON via Java" %}}

{{% blocks/products/pf/agp/text %}}

需要以程式設計方式將 SPREADSHEETML 合併到 JSON 嗎？基本文檔合併和連接[Aspose.Cells for Java](https://products.aspose.com/cells/java)只需幾行程式碼即可完成 API。
...
{{% blocks/products/pf/agp/content h2="Java 庫將 SPREADSHEETML 合併到 JSON" %}}
