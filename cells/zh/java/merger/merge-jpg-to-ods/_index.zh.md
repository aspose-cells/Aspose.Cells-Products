...
description: Aspose Excel。 Java Aspose.Cells。合并JPG到ODS保存格式。 Java 将JPG合并为ODS格式。将 JPG 合并到 Java.JPG 中的 ODS。
...
如何将 JPG 合并到 ODS？借助 Aspose.Cells for Java 库，您可以通过几行代码以编程方式轻松将 JPG 合并到 ODS。[Aspose.Cells for Java](https://products.aspose.com/cells/java)能够构建跨平台应用程序，能够生成、修改、转换、渲染和打印所有 Excel 文件。 Java Excel API 不仅可以在电子表格格式之间进行转换，还可以将 Excel 文件呈现为图像, PDF, HTML, ODS, CSV, SVG, JSON, WORD, PPT等等，从而使其成为以行业标准交换文档的完美选择格式。您可以直接从以下位置下载其最新版本[Maven](https://repository.aspose.com/webapp/#/artifacts/browse/tree/General/repo/com/aspose/aspose-cells)并通过将以下配置添加到 pom.xml 将其安装在基于 Maven 的项目中。
...
{{% blocks/products/pf/agp/content h2="将JPG合并到Java中的ODS" %}}
