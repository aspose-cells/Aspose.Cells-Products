...
1. 使用 IWorkbook 類加載 XLSM 文件
1. 使用帶有 ProtectionType 和 Password 的 Protect(..) 方法
1. 通過 Save() 方法保存被保護的 XLSM 文件
