...
{{% blocks/products/pf/agp/content h2="如何使用 Python via .Net 將旋轉按鈕（表單控件）插入 Excel 文件" %}}

為了將旋轉按鈕（表單控件）插入到 Excel 文件中，我們將使用
...
{{% blocks/products/pf/agp/feature-section-col title="使用 Python via .Net 將旋轉按鈕（表單控件）插入 Excel 文件的步驟" %}}
...
+ 通過索引選擇工作表。
 使用形狀集合的添加[方法](https://reference.aspose.com/cells/python-net/aspose.cells.drawing/shapecollection/add_spinner/#int-int-int-int-int-int)在選定的工作表中插入旋轉按鈕（表單控件）。
以 XLSX 格式保存工作簿。
...
{{% blocks/products/pf/agp/code-block title="插入旋轉按鈕（表單控件） - Python via .Net" offSpacer="" %}}
