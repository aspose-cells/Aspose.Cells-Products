---
title: PHP SPREADSHEETML을 BMP로 변환 - SPREADSHEETML을 BMP로 변환
description: Aspose Excel. 이 포괄적인 솔루션은 PHP 개발자에게 SPREADSHEETML을 BMP 형식으로 변환하는 완벽하게 통합된 방식을 제공하며, 효율적이고 사용자 정의 가능한 PHP 코드를 통해 Aspose.Cells 라이브러리를 사용하여 SPREADSHEETML 데이터를 BMP 형식으로 원활하게 저장할 수 있도록 지원합니다.
...
{{% blocks/products/pf/agp/content h2="PHP를 사용하여 SPREADSHEETML을 BMP로 변환하는 방법" %}}

{{% blocks/products/pf/agp/text %}}

SPREADSHEETML 파일을 BMP 형식으로 프로그램적으로 변환해야 하시나요? PHP 개발자는 단 몇 줄의 코드로 SPREADSHEETML 파일을 BMP 형식으로 쉽게 불러오고 변환할 수 있습니다.
...
1.  Workbook의 save 메서드를 호출하여 SPREADSHEETML을 BMP 형식으로 변환합니다.
