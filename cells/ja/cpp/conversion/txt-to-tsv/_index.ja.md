...
{{% blocks/products/pf/agp/text %}}
 ...
 C++開発者は、わずか数行のコードでTXTファイルをTSVに簡単に変換できます。

{{% /blocks/products/pf/agp/text %}}

1. Factory::CreateIWorkbookを使用してTXTファイルをロードします。1. Save（）メソッドを呼び出します。1. （TSV）ファイル拡張子を持つ出力ファイルパスを渡します。1. TSVファイルは指定されたパスに保存されます。1. 互換性のあるプログラムでTSVファイルを開きます。
...
{{% blocks/products/pf/agp/code-block title="TXTからTSVC++への変換ソースコード" offSpacer="" %}}
