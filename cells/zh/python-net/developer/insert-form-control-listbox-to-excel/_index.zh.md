...
{{% blocks/products/pf/agp/content h2="如何使用 Python via .Net 将列表框（表单控件）插入 Excel 文件" %}}
...
{{% blocks/products/pf/agp/feature-section-col title="使用Python via .Net将ListBox（表单控件）插入Excel文件的步骤" %}}
...
+ 通过索引选择工作表。
 使用形状集合的添加[方法](https://reference.aspose.com/cells/python-net/aspose.cells.drawing/shapecollection/add_list_box/#int-int-int-int-int-int)在选定的工作表中插入列表框（表单控件）。
以 XLSX 格式保存工作簿。
...
{{% blocks/products/pf/agp/code-block title="插入列表框（表单控件） - Python via .Net" offSpacer="" %}}
