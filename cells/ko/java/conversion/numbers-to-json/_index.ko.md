---
title: Java을(를) 통해 NUMBERS를 JSON으로 변환 

description: NUMBERS 형식을 JSON 파일로 변환하는 샘플 Java 변환 코드. 프로그래머는 이 예제 코드를 사용하여 Excel 및 OpenOffice 스프레드시트를 웹 또는 데스크톱 Java 기반 애플리케이션 내에서 JSON으로 내보낼 수 있습니다.
---
{{< blocks/products/pf/main-wrap-class isAutogenPage="true" >}}
{{< blocks/products/pf/i18n/upper-banner h1="Java을(를) 통해 NUMBERS를 JSON으로 변환" h2="온프레미스 Java 라이브러리를 사용하여 단일 또는 여러 페이지를 JSON으로 변환하려면 NUMBERS를 JSON Java으로 변환합니다." logoImageSrc="https://www.aspose.cloud/templates/aspose/img/products/cells/aspose_cells-for-java.svg" sourceAdditionalConversionTag="" additionalConversionTag="JSON" pfName="" subTitlepfName="" downloadUrl="" fileiconsmall1="DOCX" fileiconsmall2="JPG" fileiconsmall3="PDF" fileiconsmall4="XML" fileiconsmall5="NUMBERS" >}}
...
{{% blocks/products/pf/agp/content h2="Java을 사용하여 NUMBERS를 JSON으로 변환하는 방법" %}}

 NUMBERS를 JSON으로 렌더링하기 위해 다음을 사용합니다.
...
{{% blocks/products/pf/agp/feature-section-col title="Java을(를) 통해 NUMBERS를 JSON으로 변환하는 단계" %}}

{{% blocks/products/pf/agp/text %}}

 Java 개발자는 몇 줄의 코드로 NUMBERS 파일을 JSON으로 쉽게 변환할 수 있습니다.

{{% /blocks/products/pf/agp/text %}}

1. Workbook 클래스의 인스턴스로 NUMBERS개 파일 로드1. Cells.createRange 메소드를 사용하여 내보낼 데이터 범위 생성1. Range 및 ExportRangeToJsonOptions에 대한 참조로 JsonUtility.exportRangeToJson 메서드를 호출합니다.1. BufferedWriter.write 메서드를 통해 문자열 JSON 데이터를 파일에 씁니다.
...
{{% blocks/products/pf/agp/code-block title="NUMBERS에서 JSON으로 Java 변환 소스 코드" offSpacer="" %}}
