---
title: C++ PRN 轉 MHT - PRN 轉 MHT 轉換器
description: Aspose Excel。此綜合解決方案為 C++ 開發人員提供了一種完全整合的方法，可將 PRN 轉換為 MHT 格式，從而能夠使用 Aspose.Cells 庫將 PRN 資料無縫儲存為 MHT 格式，所有這些都透過高效且可自訂的 C++ 程式碼實現。
...
{{% blocks/products/pf/agp/content h2="將 PRN 儲存到 MHT 中，輸入 C++" %}}

以下範例示範如何在 C++ 中將 PRN 轉換為 MHT。
...
需要透過程式將 PRN 檔案轉換為 MHT 檔案嗎？ C++ 開發人員只需幾行程式碼即可輕鬆將 PRN 檔案轉換為 MHT 檔案。
...
1. 使用工作簿載入PRN檔案。
1. 呼叫 Save() 方法將 PRN 轉換為 MHT。
1. 取得 PRN 到 MHT 的轉換結果。

{{% /blocks/products/pf/agp/content %}}

{{% blocks/products/pf/agp/content h2="C++ 庫用於將 PRN 轉換為 MHT" %}}
