...
{{% blocks/products/pf/agp/text %}}
 ...
 Vývojáři C++ mohou snadno převést soubor JSON na DOCX pomocí několika řádků kódu.

{{% /blocks/products/pf/agp/text %}}

1. Načtěte soubor JSON pomocí Factory::CreateIWorkbook.1. Zavolejte metodu Save().1. Předejte cestu k výstupnímu souboru s příponou (DOCX).1. Soubor DOCX bude uložen do zadané cesty.1. Otevřete soubor DOCX v kompatibilním programu.
...
{{% blocks/products/pf/agp/code-block title="Zdrojový kód převodu JSON na DOCX C++" offSpacer="" %}}

```cs
// Načtěte JSON.
intrusive_ptr<Aspose::Cells::IWorkbook> wkb = Factory::CreateIWorkbook(u"sourceFile.json");

// Uložit ve formátu DOCX.
