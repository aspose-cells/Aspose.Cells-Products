...
weight: 810
description: C++ 示例代码，用于在 Excel 文件中插入标注，适用于 C++ 运行时环境，支持 Windows 32 位、Windows 64 位和 Linux 64 位。
...
{{% blocks/products/pf/agp/content h2="如何使用 C++ 在 Excel 文件中插入标注" %}}

为了在 Excel 文件中插入标注，我们将使用
...
+ 通过索引选择工作表。
 + 使用[添加方法](https://reference.aspose.com/cells/cpp/aspose.cells.drawing/shapecollection/addautoshape/)在选定的工作表中插入标注
将工作簿保存为 XLSX 格式。
...
以下示例代码演示了如何插入“想法气泡：云朵”。更多类型，请参阅下方的“标注类型概述”。
...
自旋形状类型::<br>AutoShapeType_LineCalloutWithBorderAndAccentBar2
   ...
   <div class="col-lg-4">
    ...
    <p class="col-lg-10" style="font-size:0.8rem !important;">
自旋形状类型::<br>AutoShapeType_LineCalloutWithBorderAndAccentBar3
    </p>
   </div>
  </div>
 </div>
